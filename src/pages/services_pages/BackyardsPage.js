...
import {
  ...
  plans,
  activities
} from '../../data/backyards_page/backyardsServiceData';

const BackyardsPage = () => {
  const [selected, setSelected] = useState([]);
  const [selectedBackyardType, setSelectedBackyardType] = useState(backyardTypes[0]);
  ...
  const getTotalPrice = () =>
    selected.reduce((total, name) => {
      ...
      return total + (found ? found.prices[selectedBackyardType][subscriptionType] : 0);
    }, 0);
    ...
    <CenteredLayout>
      <section className="service-detail-section py-5 position-relative">
        <Container className={step !== 'main' ? 'content-blurred' : ''}>
          <h2 className="text-center mb-4 service-title">Почистване на дворове</h2>

          {/* Backyard Type Selector */}
          <Row className="mb-3 justify-content-center" data-aos="fade-right">
            <Col md={6}>
              <Form.Group controlId="backyardTypeSelect">
                <Form.Label><strong>Размер на двора:</strong></Form.Label>
                <Form.Select
                  value={selectedBackyardType}
                  onChange={(e) => setSelectedBackyardType(e.target.value)}
                >
                  {backyardTypes.map((type, i) => (
        ...
        <ReservationModal
          ...
          selected={selected}
          selectedType={selectedBackyardType}
          subscriptionType={subscriptionType}
          selectedPlan={selectedPlan}
          activities={activities}
          totalPrice={getTotalPrice()}
          serviceType="backyards"
