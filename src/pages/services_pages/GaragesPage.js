...
import {
  ...
  plans,
  activities
} from '../../data/garages_page/garagesServiceData';

const GaragesPage = () => {
  const [selected, setSelected] = useState([]);
  const [selectedGarageType, setSelectedGarageType] = useState(garageTypes[0]);
  ...
  const getTotalPrice = () =>
    selected.reduce((total, name) => {
      ...
      return total + (found ? found.prices[selectedGarageType][subscriptionType] : 0);
    }, 0);
    ...
    <CenteredLayout>
      <section className="service-detail-section py-5 position-relative">
        <Container className={step !== 'main' ? 'content-blurred' : ''}>
          <h2 className="text-center mb-4 service-title">Почистване на гаражи и паркоместа</h2>

          {/* Garage Type Selector */}
          <Row className="mb-3 justify-content-center" data-aos="fade-right">
            <Col md={6}>
              <Form.Group controlId="garageTypeSelect">
                <Form.Label><strong>Размер на зоната:</strong></Form.Label>
                <Form.Select
                  value={selectedGarageType}
                  onChange={(e) => setSelectedGarageType(e.target.value)}
                >
                  {garageTypes.map((type, i) => (
        ...
        <ReservationModal
          ...
          selected={selected}
          selectedType={selectedGarageType}
