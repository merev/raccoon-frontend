...
import {
  ...
  plans,
  activities
} from '../../data/venues_page/venuesServiceData';
...
const VenuesPage = () => {
  ...
  const getTotalPrice = () =>
    selected.reduce((total, name) => {
      ...
      return total + (found ? found.prices[selectedVenueType][subscriptionType] : 0);
    }, 0);
    ...
    <CenteredLayout>
      <section className="service-detail-section py-5 position-relative">
        <Container className={step !== 'main' ? 'content-blurred' : ''}>
          <h2 className="text-center mb-4 service-title">Почистване на заведения</h2>

          {/* Venue Type Selector */}
          <Row className="mb-3 justify-content-center" data-aos="fade-right">
            <Col md={6}>
              <Form.Group controlId="venueTypeSelect">
