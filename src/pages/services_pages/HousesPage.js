...
import {
  ...
  plans,
  activities
} from '../../data/houses_page/housesServiceData';
...
const HousesPage = () => {
  ...
  const getTotalPrice = () =>
    selected.reduce((total, name) => {
      ...
      return total + (found ? found.prices[selectedHouseType][subscriptionType] : 0);
    }, 0);
    ...
    <CenteredLayout>
      <section className="service-detail-section py-5 position-relative">
        <Container className={step !== 'main' ? 'content-blurred' : ''}>
          <h2 className="text-center mb-4 service-title">Почистване на къщи</h2>

          {/* House Type Selector */}
          <Row className="mb-3 justify-content-center" data-aos="fade-right">
            <Col md={6}>
              <Form.Group controlId="houseTypeSelect">
                <Form.Label><strong>Тип на къщата:</strong></Form.Label>
                <Form.Select
