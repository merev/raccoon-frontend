...
import {
  ...
  plans,
  activities
} from '../../data/special_objects_page/specialObjectsServiceData';

const SpecialObjectsPage = () => {
  const [selected, setSelected] = useState([]);
  const [selectedObjectType, setSelectedObjectType] = useState(objectTypes[0]);
  ...
  const getTotalPrice = () =>
    selected.reduce((total, name) => {
      ...
      return total + (found ? found.prices[selectedObjectType][subscriptionType] : 0);
    }, 0);
    ...
    <CenteredLayout>
      <section className="service-detail-section py-5 position-relative">
        <Container className={step !== 'main' ? 'content-blurred' : ''}>
          <h2 className="text-center mb-4 service-title">Почистване на специализирани обекти</h2>

          {/* Object Type Selector */}
          <Row className="mb-3 justify-content-center" data-aos="fade-right">
            <Col md={6}>
              <Form.Group controlId="objectTypeSelect">
                <Form.Label><strong>Тип на обекта:</strong></Form.Label>
                <Form.Select
...
export default SpecialObjectsPage;
