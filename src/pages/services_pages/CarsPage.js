...
import {
  ...
  plans,
  activities
} from '../../data/cars_page/carsServiceData';
...
const CarsPage = () => {
  ...
  const getTotalPrice = () =>
    selected.reduce((total, name) => {
      ...
      return total + (found ? found.prices[selectedCarType][subscriptionType] : 0);
    }, 0);
    ...
    <CenteredLayout>
      <section className="service-detail-section py-5 position-relative">
        <Container className={step !== 'main' ? 'content-blurred' : ''}>
          <h2 className="text-center mb-4 service-title">Почистване на автомобили</h2>

          {/* Car Type Selector */}
          <Row className="mb-3 justify-content-center" data-aos="fade-right">
            <Col md={6}>
              <Form.Group controlId="carTypeSelect">
                <Form.Label><strong>Тип на автомобила:</strong></Form.Label>
                <Form.Select
